...
if($_REQUEST['mysession']){
	$meli = new Meli('4270558986407679', 'ArbqnANwnlxz9W0EXwnfvb0niBAWZhGf', $_REQUEST['mysession']);
...
function getSessionToken($ch){
	...
	curl_setopt($ch, CURLOPT_URL, "https://geeksuy.com/ml/auth.php");
...
function gotToSite($session,$ch){
	curl_setopt($ch, CURLOPT_URL, "https://geeksuy.com/ml/auth.php?mysession=".$session);
	curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
	curl_setopt($ch, CURLOPT_POST, false);
	curl_setopt($ch, CURLOPT_POSTFIELDS, "");
	$answer = curl_exec($ch);
